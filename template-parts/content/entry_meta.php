...
<div class="entry-meta">
	...
	if ( ! empty( $author_string ) ) {
		...
		<span class="posted-by">
			...
			if ( class_exists( 'BuddyPress' ) ) {
				...
				printf( _x( 'Written by %s', 'post author', 'buddyxpro' ), bp_core_get_userlink( $post->post_author ) );
			}else {
				echo get_avatar( get_the_author_meta('user_email'), $size = '38');
				$author_byline = _x( 'Written by %s', 'post author', 'buddyxpro' );
			}
	...
	if ( ! empty( $parent_string ) ) {
		...
		<span class="posted-in">
			...
			$parent_note = _x( 'In %s', 'post parent', 'buddyxpro' );
	...
	if ( ! empty( $time_string ) ) {
		...
		<span class="posted-on">
			...
			printf(
				...
				esc_html_x( '%s', 'post date', 'buddyxpro' ),

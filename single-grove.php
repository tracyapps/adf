...
?>
	...
	<main id="primary" class="site-main">
		...
		if ( is_user_logged_in() && is_single( 'saved-favorites' ) ){
			if ( get_user_favorites( ) ) :
				the_user_favorites_list($user_id, $site_id, $include_links = true, $filters, $include_button = false, $include_thumbnails = true, $thumbnail_size = 'thumbnail', $include_excerpt = true);
				printf(
					'<footer class="favorites-list-footer">
							<p>To clear all your saved favorites, click the button below. However, this cannot be undone, so proceed with caution!</p>
							 <p>%s</p>
							 </footer>
							 ',
					get_clear_favorites_button($site_id, $text="Clear all favorites")
				);
			else :
